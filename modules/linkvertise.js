...
if (settings.linkvertise.enabled) {
    ...
    app.get(`/lv/gen`, async (req, res) => {
        ...
        if (!referer.endsWith(`/lv`) && !referer.endsWith(`/lv/`)) return res.redirect('/lv')
    ...
    app.get(`/lv/redeem`, async (req, res) => {
        ...
        if (!req.headers.referer || !req.headers.referer.includes('linkvertise.com')) return res.redirect('/lv?err=BYPASSER')
        ...
        if (((Date.now() - usercode.generated) / 1000) < settings.linkvertise.minTimeToComplete) {
            return res.redirect('/lv?err=BYPASSER')
        }
